...
options:
    ...
    wait_until_inactive:
        description:
            - When deleting a service, wait for service to reach an INACTIVE state.  When deleting a service, the service will first transition from an ACTIVE state to a DRAINING state, and then to an INACTIVE state when all client connections to the service have closed.  Default value is True. 
        required: False
'''
